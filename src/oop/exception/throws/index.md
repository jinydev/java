...
*   **`Throw`**: 쓰로우, 던지다. (문제 상황을 발견했을 때 "앗, 에러다!" 하고 직접 빨간색 경고탄(예외 객체)을 하늘로 뻥 쏘아 올리는 행위)
*   **`Throws`**: 쓰로우즈, 떠넘기다. (메소드 이름 옆에 찰싹 붙어서, "내 안에서 일어나는 사고는 내가 수습 안 하고 나를 부른 쪽으로 던져버릴 테니 네가 알아서 해!"라고 선언하는 무책임해 보이지만 합리적인 책임 전가 키워드)
