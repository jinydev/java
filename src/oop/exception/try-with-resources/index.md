...
description: "14.4 리소스 자동 닫기 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "14.4 리소스 자동 닫기, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Resource`**: 리소스, 자원. (프로그램 밖에서 빌려 쓰는 소중한 외부 물건들. 예를 들어 텍스트 파일, 이미지 버퍼, 외부 인터넷 망 등 다 쓰면 꼭 반납해야 하는 것들)
*   **`Auto Closeable`**: 오토 클로저블, 자동으로 닫을 수 있는. (이 능력을 갖춘 특별한 객체들은 `try( )` 괄호 안에 들어가기만 하면, 사용이 끝났을 때 누가 시키지 않아도 스스로 문을 닫고(`close`) 깔끔하게 퇴장하는 성질)
