...
description: "18.3 매개변수가 있는 람다식 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "18.3 매개변수가 있는 람다식, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Parameter`**: 파라미터, 매개변수. (요리를 하기 위해 레시피(람다식)에 던져주는 당근, 양파 같은 외부 입력 데이터. `(x)` 형태로 받음)
*   **`Type Inference`**: 타입 인퍼런스, 타입 추론. (`(String a)` 대신 `(a)`라고 써도 자바 컴파일러가 똑똑하게 문맥을 보고 "아, 이건 문자열이구나" 하고 알아맞히는 기능)
