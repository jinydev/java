...
*   **`Anonymous`**: 어노니머스, 익명의. (이름이 없는 객체. 원래 구현 클래스를 만들려면 이름 짓고 설계도 짜야 하는데, 람다식은 그저 이름 없이 로직만 훌쩍 던져주는 '익명 구현 객체'임)
*   **`Functional Interface`**: 펑셔널 인터페이스, 함수형 인터페이스. (수많은 추상 메소드도 안 되고, 딱! 한 개의 추상 메소드만 가지고 있어서 람다식으로 안전하게 1:1 교환이 가능한 인터페이스)
