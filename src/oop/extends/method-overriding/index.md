...
description: "7.4 메소드 재정의 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "7.4 메소드 재정의, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Overriding`**: 오버라이딩, 재정의. (부모 클래스에게 물려받은 원본 메소드(기능)를 자식 클래스가 자기 입맛에 맞게 통째로 다시 덮어써서(Overwrite) 새롭게 정의하는 것)
*   **`Virtual Method Table (V-Table)`**: 가상 메소드 테이블. (객체가 어떤 버전의 메소드(부모 것인지 내 것인지)를 실행할지 런타임에 결정할 때 참고하는 내부 주소록과 같은 개념)
