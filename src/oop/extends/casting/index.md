...
description: "7.7 타입 변환 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "7.7 타입 변환, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Casting`**: 캐스팅, 타입 변환. (객체의 참조 타입을 부모나 자식 타입으로 바꾸어, 메모리를 바라보는 시야나 자격을 조절하는 것)
*   **`Upcasting`**: 업캐스팅. (자식 객체가 더 넓은 범위인 부모 타입으로 자연스럽고 안전하게 올라가는 자동 타입 변환)
*   **`Downcasting`**: 다운캐스팅. (부모 타입으로 묶여 있던 객체를 괄호 `()`를 써서 다시 원래의 구체적인 자식 타입으로 강제로 끌어내리는 변환)
