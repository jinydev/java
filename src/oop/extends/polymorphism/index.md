...
description: "7.8 다형성 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "7.8 다형성, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Polymorphism`**: 폴리모피즘, 다형성. (상속받은 여러 자식 객체들이 하나의 부모 타입이라는 똑같은 리모컨 버튼(메소드명)을 눌러도, 각자 자기만의 다양한 형태(기능)로 알아서 동작하는 성질)
*   **`Coupling`**: 커플링, 결합도. (하나의 클래스가 다른 클래스가 어떻게 생겼는지 얼마나 자세히 알고 의존하는지를 나타내는 정도. 다형성을 쓰면 이 결합도를 낮춰(Loose) 유지보수가 아주 쉬워짐)
