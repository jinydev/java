...
description: "12.1 중첩 클래스 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "12.1 중첩 클래스, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Nested`**: 네스티드, 중첩된. (마치 러시아 전통 인형 마트료시카처럼, 하나의 커다란 껍질(바깥 클래스) 속에 또 다른 작은 알맹이들(안쪽 클래스)이 차곡차곡 포개져 들어있는 우아한 구조)
*   **`Member`**: 멤버, 구성원. (단순한 부품이 아니라 바깥 클래스라는 한 가족의 당당한 일원으로서 필드, 메소드와 어깨를 나란히 하며 소속감을 자랑하는 클래스)
