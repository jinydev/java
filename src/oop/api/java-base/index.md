...
*   **`Module`**: 모듈. (클래스들을 모은 것이 패키지라면, 그 패키지들을 다시 쓰임새에 맞게 거대한 덩어리로 잘 묶어놓은 레고 상자. 자바 9부터 도입된 개념)
*   **`Require`**: 리콰이어, 요구하다, 필요로 하다. (보통 다른 레고 상자(모듈)를 쓰려면 허락을 받아야 하지만, `java.base` 만큼는 물이나 공기처럼 기본 제공이라 유일하게 허락받지 않고도 막 쓸 수 있음)
