---
layout: oop
title: "12.2 java.base 모듈"
nav_order: 2
parent: "Chapter 12. java.base 모듈"
grand_parent: "객체지향 프로그래밍"
description: "12.2 java.base 모듈 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "12.2 java.base 모듈, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---

# 12.2 java.base 모듈

`java.base`는 모든 모듈이 의존하는 `기본 모듈`로, 모듈 중 유일하게 `requires`하지 않아도 사용할 수 있다. 이 모듈에 포함되어 있는 패키지는 대부분의 자바 프로그램에서 많이 사용하는 것들이다. 다음은 `java.base` 모듈에 포함된 주요 패키지와 용도를 설명한 표이다.
...
`java.lang`은 자바 언어의 기본적인 클래스를 담고 있는 패키지로, 이 패키지에 있는 클래스와 인터페이스는 `import 없이 사용`할 수 있다. 다음은 `java.lang` 패키지에 포함된 주요 클래스와 용도를 설명한 표이다.
