...
description: "15.5 문자열 클래스 (String) 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "15.5 문자열 클래스 (String), 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Immutable`**: 이뮤터블, 불변의. (한 번 태어난 `String` 글자 객체는 메모리 안에서 죽을 때까지 형태를 바꿀 수 없어서, 글자를 더하면 아예 새로운 객체가 쪼개져 나오는 자바 문자열의 고집 센 특징)
*   **`Token`**: 토큰, 조각. (`StringTokenizer`를 사용해 "피자/치킨/콜라" 같은 긴 주문 내역을 `/` 모양 칼(구분자)로 썰어냈을 때 떨어져 나오는 각각의 맛있는 맛 단위 조각들)
