...
*   **`Object`**: 오브젝트, 객체, 사물. (특별히 상속(`extends`)을 적지 않아도, 자바 세상에 태어나는 모든 클래스들이 본능적으로 물려받는 최초의 단군 할아버지 클래스)
*   **`Hash Code`**: 해시 코드. (수많은 객체들 중 똑같은 쌍둥이(동일인)인지 빠르게 찾아내기 위해, 객체마다 메모리 번지를 섞어 발급해 주는 고유한 암호나 주민등록번호 같은 `int` 정수값)
