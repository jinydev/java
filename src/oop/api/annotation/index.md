...
*   **`Annotation`**: 어노테이션, 주석. (단순히 사람 보라고 적는 메모가 아니라, `@` 기호를 붙여 컴파일러나 프레임워크(Spring 등)에게 "이 클래스는 특별하게 처리해 줘!" 하고 남기는 강력한 시스템 설정 태그)
*   **`Retention`**: 리텐션, 유지, 보유. (내가 달아놓은 어노테이션 태그가 컴파일할 때까지만 살아있을지, 아니면 프로그램이 쌩쌩 돌아가는(`RUNTIME`) 중에도 끈질기게 살아남아 활용될지를 결정하는 유지 정책)
