...
*   **`Local`**: 로컬, 지역의. (`LocalDateTime`에서 컴퓨터가 켜져 있는 현재 위치(국가, 지역)의 시간대 포맷에 맞춰 날짜와 시간을 보여주는 스마트한 최신 자바 시간 객체)
*   **`Calendar`**: 캘린더, 달력. (과거에 많이 썼지만, 사람이 쓰는 1월을 컴퓨터는 `0`으로 기억하는 치명적인 불편함 때문에 요즘은 서랍 속에 넣어둔 구식 자바 달력 클래스)
