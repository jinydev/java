...
*   **`Reflection`**: 리플렉션, 투영, 반사. (마치 거울에 객체를 비춰보듯, 작동 중인 객체의 뱃속을 몰래 까보고 어떤 필드와 메소드가 들었는지 알아내거나 심지어 숨겨진(`private`) 값까지 끄집어내는 강력하고 짜릿한 고급 해킹 기술)
*   **`Meta`**: 메타. (데이터를 설명하는 데이터. 즉, 객체가 가진 '알맹이 값'이 아니라, 그 객체의 '이름이 뭔지', '부모는 누군지' 등 프로필 정보를 담고 있는 `Class` 객체의 정보를 뜻함)
```
