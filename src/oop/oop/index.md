...
description: "Chapter 09. 클래스 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "Chapter 09. 클래스, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Object-Oriented Programming (OOP)`**: 객체지향 프로그래밍. (거대한 프로그램을 만들 때 아무렇게나 코드를 섞어 짜는 게 아니라, 독립된 부품(객체)들을 하나하나 만들고 이들을 조립해서 시스템을 완성해가는 현대적인 소프트웨어 개발 방식)
*   **`Object`**: 오브젝트, 객체, 사물. (소프트웨어 세계 안에서 구현하고자 하는 구체적인 형태를 띤 모든 것. 예: 플레이어, 몬스터, 총알, 자동차 등)
*   **`Oriented`**: 오리엔티드, ~지향의. (방향성이나 목표가 무언가를 중심에 두고 향하고 있다는 뜻)
