...
*   **`Static`**: 스태틱, 정적인/고정된. (객체마다 따로따로 갖지 않고, 클래스 원본 자체에 딱 하나만 찰싹 고정되어 만들어진 후 모든 객체가 다 함께 돌려 쓰는 공용 멤버를 만들 때 붙이는 딱지)
*   **`Member`**: 멤버. (클래스라는 커다란 한 지붕 아래에 모여 살며 각자의 역할을 수행하는 필드, 메소드, 생성자 등의 모든 내부 구성원들)
