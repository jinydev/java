...
접근 제한자는 `public`, `protected`, `private`와 `default`(아무것도 붙이지 않음)가 있다.
...
라이브러리 클래스를 개발할 때에는 일반적으로 `public` 접근 제한을 갖도록 해서 어느 패키지에서나 사용할 수 있도록 한다. 그러나 인터넷으로 배포되는 라이브러리 클래스들 중에는 `public`이 아닌 것도 있다. 이것은 외부로 노출시키지 않고 라이브러리 내부에서만 사용하기 위해서이다.
