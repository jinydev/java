...
*   **`Getter`**: 게터, 접근자. (꽁꽁 숨겨진 `private` 필드의 진짜 데이터를 외부로 꺼내서 안전하게 보여주기만 하는 읽기 전용 통로 메소드. 이름이 보통 `get`으로 시작함)
*   **`Setter`**: 세터, 설정자. (외부에서 들어온 값이 정상적인지 앞단에서 검사하고 걸러내어, 안전할 때만 내부 필드에 변경을 덮어쓰는 쓰기 전용 통로 메소드. 이름이 보통 `set`으로 시작함)
*   **`Integrity`**: 인테그리티, 무결성. (자동차 속도가 마이너스가 될 수 없듯, 데이터가 흠집 없이 오작동하지 않는 완전무결한 논리적 정상 상태를 유지하는 것)
