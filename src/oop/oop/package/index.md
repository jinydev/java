...
*   **`Package`**: 패키지. (수많은 자바 클래스 파일들이 뒤섞여 엉망이 되지 않도록 종류나 기능별로 체계적으로 묶어두는 파일 시스템의 폴더(디렉토리))
*   **`import`**: 임포트, 수입하다/가져오다. (다른 동네(패키지)에 살고 있는 클래스를 우리 동네에서도 이름만 짧게 부르며 편하게 쓰겠다고 컴파일러에게 미리 알려주는 마법의 명함)
