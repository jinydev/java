...
description: "6.8 메소드 선언과 호출 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "6.8 메소드 선언과 호출, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Method`**: 메소드. (객체가 수행할 수 있는 동작이나 기능을 정의해둔 코드 블록. 현실 세계의 '동사'나 '명령'에 해당함)
*   **`Return`**: 리턴, 반환. (메소드가 묵묵히 일을 끝마친 뒤, 자신을 호출하고 기다리는 쪽으로 그 결과물을 챙겨서 돌려주는 행위)
*   **`Parameter`**: 파라미터, 매개변수. (메소드라는 블랙박스 자판기에 동전이나 필수 재료를 밀어 넣기 위해 뚫어 놓은 투입구 역할의 변수)
