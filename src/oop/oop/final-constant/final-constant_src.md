...
`final`은 '최종적'이란 뜻을 가지고 있다. `final` 필드는 초기값이 저장되면 이것이 최종적인 값이 되어서 프로그램 실행 도중에 수정할 수 없게 된다. `final` 필드는 다음과 같이 선언한다.
...
다음 예제에서 `Korean` 클래스를 보면 국가(`nation`)와 주민등록번호(`ssn`) 필드를 `final` 필드로 선언했다. `nation`은 고정값이므로 선언 시에 초기값을 대입했고, `ssn`은 `Korean` 객체가 생성될 때 부여되므로 생성자 매개값으로 주민등록번호를 받아 초기값으로 대입했다.
...
상수는 객체마다 저장할 필요가 없고, 여러 개의 값을 가져도 안 되기 때문에 `static`이면서 `final`인 특성을 가져야 한다. 따라서 상수는 다음과 같이 선언한다.
