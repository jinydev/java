...
description: "Chapter 11. 인터페이스 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "Chapter 11. 인터페이스, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Interface`**: 인터페이스. (사전적으로는 두 장치를 연결하는 접속기. 자바에서는 서로 다른 객체들이 소통하기 위해 반드시 지켜야 할 공통 규격이나 약속을 정의해둔 뼈대)
*   **`Implements`**: 임플리먼츠, 구현하다. (클래스가 인터페이스에 정의된 추상적인 규칙이나 기능(메소드)들을 구체적으로 자신의 몸체 안에 완성하여 실현하겠다고 선언하는 키워드)
