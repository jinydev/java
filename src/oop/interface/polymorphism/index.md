...
*   **`Polymorphism`**: 폴리모피즘, 다형성. (똑같은 `run()` 버튼(메소드)을 눌러도, 어떤 타이어나 탈것(구현 객체)을 끼워 넣었냐에 따라 각기 다른 방식으로 부드럽게 돌아가는 마법 같은 성질)
*   **`Plug and Play`**: 플러그 앤 플레이. (마치 컴퓨터에 USB 마우스를 꽂으면 수정 없이 바로 작동하듯, 기존 코드는 한 줄도 안 고치고 새로운 부품 객체만 쏙 끼워서 바로 갈아입히는 개발 방식)
