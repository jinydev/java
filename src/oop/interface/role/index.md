...
*   **`Role`**: 롤, 역할. (인터페이스가 객체지향에서 맡은 임무로, 실제 복잡한 알맹이(객체)를 대신해 사용자(개발자)가 편하게 쓸 수 있게 해주는 중계자/리모컨 기능)
*   **`Loose Coupling`**: 루스 커플링, 느슨한 결합. (사용자가 특정 기기(객체)의 내부 부품에 얽매이지 않게 끊어주고, 오직 리모컨(인터페이스) 버튼에만 의존하게 만들어, 나중에 TV를 통째로 갈아치워도 내 코드는 끄떡없는 튼튼한 상태)
