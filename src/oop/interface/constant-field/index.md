...
*   **`Constant`**: 컨스턴트, 상수. (한 번 어떤 값으로 정해지면 그 이후로는 절대 다른 값으로 변경될 수 없는 확고부동한 불변의 고정값)
*   **`Static Final`**: 스태틱 파이널. (인터페이스의 모든 필드에 보이지 않게 자동으로 붙는 수식어로, '객체 생성 없이도 쓸 수 있으며(static) 절대 바꿀 수 없는(final)' 상태를 만듦)
