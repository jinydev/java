...
description: "13.2 모듈이란? 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "13.2 모듈이란?, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Container`**: 컨테이너. (패키지들이 이리저리 섞이지 않도록 더 큰 단위로 안전하게 묶어서 담아두는 배송용 철제 박스 같은 역할)
*   **`Manifest`**: 매니페스트, 명세서. (`module-info.java` 처럼 "이 컨테이너 안에는 뭐가 있고, 밖으로 뺄 수 있는 건 이거야" 라고 적어둔 화물 선언서)
*   **`Exports`**: 익스포트, 수출하다. (자물쇠로 굳게 잡힌 모듈 안에서 "이 패키지 기능만큼은 외부 사람들이 막 써도 좋아!" 라고 자물쇠를 열어주는(공개하는) 키워드)
