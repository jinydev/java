...
description: "13.7 집합 모듈 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "13.7 집합 모듈, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Aggregator`**: 애그리게이터, 수집기, 집합. (여러 개의 자잘한 모듈들을 하나로 크게 묶어서, 마치 '종합 선물 세트'처럼 한 번에 가져다 쓸 수 있게 해주는 껍데기 모듈)
*   **`Transitive`**: 트랜지티브, 전이되는. (`requires transitive` 라고 쓰면, "나를 부르는 애한테 내 안의 모듈들도 덤으로 다 쓸 수 있게 넘겨줄게"라는 뜻의 옵션)
