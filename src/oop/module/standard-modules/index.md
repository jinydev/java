...
*   **`Standard`**: 스탠다드, 표준. (자바(JDK)를 설치하면 기본적으로 제공되는 `java.base`, `java.desktop` 등 공식적이고 표준화된 핵심 모듈들)
*   **`Base`**: 베이스, 기초, 기반. (`java.base` 처럼 모든 자바 프로그램이 무조건 의존하며 깔고 가는 가장 뿌리가 되는 모듈)
*   **`Jlink`**: 제이링크. (거대한 자바 런타임 환경(JRE) 전체를 싣고 다닐 필요 없이, 내 프로그램에 딱 필요한 모듈만 추려내어 가벼운 맞춤형 실행 환경을 조립해 주는 똑똑한 도구)
