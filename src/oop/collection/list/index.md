...
*   **`ArrayList`**: 어레이리스트. (가장 널리 쓰이는 List. 배열(Array)처럼 생겼지만 데이터를 넣을수록 크기가 자동으로 쑥쑥 늘어나는 마법의 리스트)
*   **`LinkedList`**: 링크드리스트. (데이터들이 서로 손에 손을 잡고(Link) 기차처럼 연결되어 있어서, 중간에 끼어들거나 빠질 때 아주 날쌔게 끊고 이어 붙일 수 있는 리스트)
*   **`Vector`**: 벡터. (ArrayList의 옛날 버전이자 쌍둥이 형제. '동기화'라는 안전띠를 메고 있어서 여러 스레드(작업자)가 동시에 건드려도 안전한 리스트)
