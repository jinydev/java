...
*   **`TreeSet`**: 트리셋. (데이터를 저장하는 순간 알아서 가나다순이나 123순으로 가지런히 정렬해 주는 똑똑한 Set 보석함)
*   **`TreeMap`**: 트리맵. (Map 컬렉션이지만, Key 값을 기준으로 오름차순으로 깔끔하게 줄을 세워 저장해 두는 라커룸)
*   **`Comparable` / `Comparator`**: 컴페어러블 / 컴페어레이터, 비교기. (Tree에 객체를 넣을 때 "나이 순으로 할래? 이름 순으로 할래?" 하고 정렬의 기준(규칙)을 정해주는 심판관 역할의 인터페이스)
