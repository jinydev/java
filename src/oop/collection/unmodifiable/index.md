...
description: "17.7 수정할 수 없는 컬렉션 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "17.7 수정할 수 없는 컬렉션, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Unmodifiable`**: 언모디파이어블, 수정 불가능한. (데이터 변경을 원천 차단하여, 함부로 넣거나(`add`) 뺄 수(`remove`) 없게 박물관에 박제해버리는 불변의 컬렉션)
*   **`Unsupported`**: 언서포티드, 지원하지 않는. (수정 불가능한 컬렉션에 억지로 데이터를 끼워 넣으려고 할 때 뿜어내는 '지원하지 않는 동작입니다'라는 뜻의 깐깐한 에러 메시지)
