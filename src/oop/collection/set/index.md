...
*   **`HashSet`**: 해시셋. (주머니에 손을 넣어 아무거나 꺼내듯, 들어간 순서도 없고 중복도 얄짤없이 튕겨내는 가장 기본적이고 빠른 보석함)
*   **`LinkedHashSet`**: 링크드해시셋. (HashSet의 성질(중복 금지)은 그대로 가지면서, 데이터들끼리 끈(Link)으로 묶어두어 **"내가 넣었던 순서"**만큼은 정확히 기억해 주는 보석함)
*   **`Iterator`**: 이터레이터, 반복자. (인덱스 번호가 없는 Set 주머니 안에서 데이터를 하나씩 하나씩(next) 꺼내며 순회하게 도와주는 지휘봉 같은 도구)
