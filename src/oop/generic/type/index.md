...
description: "13.2 제네릭 타입 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "13.2 제네릭 타입, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Multi Type`**: 멀티 타입, 다중 타입. (`<K, V>`처럼 타입 파라미터를 두 개 이상 연달아 쓸 수 있어, 쌍으로 묶여 다니는 제품(종류+모델명)이나 사전(단어+뜻)을 관리할 때 아주 유용한 방식)
*   **`Interface`**: 인터페이스. (클래스뿐만 아니라 인터페이스에도 제네릭을 달아서, 렌탈샵(`Rentable<P>`)이 자동차를 빌려줄지 집을 빌려줄지 구현할 때 유연하게 정하게 해주는 역할)
