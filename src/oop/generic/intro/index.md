...
*   **`Generic`**: 제네릭. (상자를 만들 때 내용물의 타입을 뭉뚱그려 `Object`로 두지 않고, 사용할 때 `<String>`처럼 구체적인 타입을 명시해 에러를 싹 막아주는 신비한 이름표)
*   **`Parameter`**: 파라미터, 매개변수. (`<T>`에서 T를 부르는 말. 일반 파라미터가 '값'을 받는 비어있는 그릇이라면, 타입 파라미터는 '타입(클래스)'을 받는 비어있는 그릇)
