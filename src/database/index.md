...
*   **[23.1 JDBC 개요](jdbc/01/)**: JDBC의 구조와 주요 인터페이스(`Connection`, `Statement`, `ResultSet` 등)의 역할을 알아봅니다.
*   **[23.2 DBMS 설치](jdbc/02/)**: 실습에 사용할 데이터베이스(Oracle 또는 MySQL)를 설치하고 설정합니다.
*   **[23.3 Client Tool 설치](jdbc/03/)**: SQL Developer와 같은 클라이언트 도구를 설치하여 데이터베이스를 관리합니다.
*   **[23.4 DB 구성](jdbc/04/)**: 실습에 필요한 사용자 계정, 테이블, 시퀀스 등을 생성합니다.
*   **[23.5 DB 연결](jdbc/05/)**: 자바 코드에서 `DriverManager`를 사용하여 데이터베이스에 연결하는 방법을 실습합니다.
*   **[23.6 데이터 저장](jdbc/06/)**: `INSERT` 문을 실행하여 데이터를 저장하는 방법을 배웁니다.
*   **[23.7 데이터 수정](jdbc/07/)**: `UPDATE` 문을 실행하여 저장된 데이터를 수정하는 방법을 배웁니다.
*   **[23.8 데이터 삭제](jdbc/08/)**: `DELETE` 문을 실행하여 데이터를 삭제하는 방법을 배웁니다.
*   **[23.9 데이터 읽기](jdbc/09/)**: `SELECT` 문을 실행하고 `ResultSet`을 통해 결과 데이터를 읽어오는 방법을 배웁니다.
*   **[23.10 프로시저와 함수 호출](jdbc/10/)**: `CallableStatement`를 사용하여 데이터베이스에 저장된 프로시저와 함수를 호출합니다.
*   **[23.11 트랜잭션 처리](jdbc/11/)**: `commit`과 `rollback`을 사용하여 트랜잭션을 제어하는 방법을 학습합니다.
*   **[23.12 게시판 구현](jdbc/12/)**: 앞서 배운 내용을 종합하여 콘솔 기반의 게시판 애플리케이션을 구현합니다.
...
*   **[24.1 MySQL 개요 및 설치](mysql/01/)**: MySQL의 특징을 이해하고 설치합니다.
*   **[24.2 MySQL JDBC 연결](mysql/05/)**: MySQL JDBC Driver를 이용하여 연결하는 방법을 배웁니다.
*   **[24.3 MySQL 데이터 처리](mysql/06/)**: MySQL에서의 데이터 CRUD 작업을 실습합니다.
