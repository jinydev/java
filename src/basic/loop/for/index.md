...
*   **`For`**: 포. (~를 위하여, ~동안. 정해진 횟수만큼 반복할 때 쓰는 가장 대표적인 반복문)
*   **`Count / Counter`**: 카운트 / 카운터. (반복문이 몇 바퀴째 도는지 숫자를 세는 변수)
*   **`Iteration / Loop`**: 이터레이션 / 루프. (한 쳇바퀴를 도는 반복 작업 그 자체)
*   **`Nested`**: 중첩된. (for문 안에 또 다른 for문이 들어있는 형태)
