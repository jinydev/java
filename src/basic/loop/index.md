...
description: "07. 반복문 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "07. 반복문, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
CPU는 심장 박동처럼 **클럭(Clock)** 이라는 전기 신호에 맞춰 1초에 수십억 번씩 규칙적으로 뜁니다 (예: 3.0GHz 칩셋).
우리가 탈출 조건(예: 10바퀴만 돌아라)을 정해주지 않는다면, CPU는 전원이 켜져 있는 한 영원히 이 클럭 사이클을 돌며 쳇바퀴 안의 코드를 반복 실행합니다. 

이 반복 과정은 단순히 뱅글뱅글 도는 것이 아니라, **4단계의 치밀한 구조**를 가집니다:

![CPU Clock 제어문 애니메이션](./img/loop_cpu_clock_anim.svg)

*   **① 초기화 (Init, 빨간색)**: 햄스터가 달리기 전 "나는 0바퀴부터 시작할거야" 라고 카운터를 0으로 맞춥니다. (루프 진입 시 딱 1번만 실행됨)
*   **② 조건 판단 (Condition, 노란색)**: "지금 내가 10바퀴를 채웠나?" 매번 뛰기 전에 검사합니다. (참이면 3번으로 가고, 거짓이면 쳇바퀴를 강제로 탈출하여 루프를 종료합니다!)
*   **③ 코드 실행 (Execution, 초록색)**: 진짜로 달립니다. (우리가 `{}` 중괄호 안에 시킨 진짜 목적 달성 코드 1회 실행)
*   **④ 증감 (Increment, 보라색)**: 1바퀴를 뛰었으므로, 카운터를 `+1` 시킵니다. 그리고 다시 ②번(조건 검사)으로 돌아가 사이클을 이어갑니다.
