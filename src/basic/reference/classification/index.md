...
*   **`Primitive Type`**: 기본 타입, 원시 타입. (정수, 실수, 논리값 등 8가지의 실제 데이터를 변수 주머니에 직접 보관하는 가벼운 데이터 타입)
*   **`Reference Type`**: 참조 타입. (배열, 문자열, 클래스 등 거대한 데이터 객체의 메모리 번지수(주소)표만 변수에 보관하는 데이터 타입)
*   **`Reference`**: 참조. (메모리 상의 객체 위치나 주소를 가리킴을 의미)
