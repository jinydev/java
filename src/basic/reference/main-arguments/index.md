...
*   **`Argument (args)`**: 아규먼트, 인자, 인수. (메소드를 호출할 때 바깥에서 실제로 던져주는 '진짜 값'. 보통 약자인 `args` 로 사용함)
*   **`Parameter (param)`**: 파라미터, 매개변수. (던져주는 값을 받기 위해 선언해 둔 '빈 그릇/수신함'. `param`으로 줄여 씀)
*   **`Parse`**: 파스, 분석하다, 변환하다. (단순 문자열을 의미 있는 숫자 데이터로 분해하고 재조립할 때 쓰는 단어)
*   **`Format`**: 포맷, 형식, 형태.
*   **`Exception`**: 익셉션, 예외. (프로그램이 예상치 못한 형식이나 값을 만나 터지는 에러/사고)
