...
*   **`Equality`**: 동등성. (안에 들어있는 실제 내용물(값)이 똑같은지 여부를 나타냄. 예: 일란성 쌍둥이)
*   **`Identity`**: 동일성. (주민등록번호나 메모리 번지수처럼 완전히 똑같은 단 하나의 객체인지 여부를 나타냄. 예: 도플갱어가 아닌 나 자신)
*   **`equals`**: 이퀄스. (참조 타입 변수들이 가진 실제 알맹이 데이터가 똑같은지 깐깐하게 비교할 때 쓰는 필수 비교 로직)
