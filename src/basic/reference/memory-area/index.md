...
*   **`Memory`**: 메모리. (데이터를 저장하는 컴퓨터의 핵심 기억 공간)
*   **`Stack`**: 스택. (나중에 들어온 것이 먼저 나가는 구조의 메모리 영역. 메서드 실행 시 지역 변수가 잠시 쌓였다가 싹 지워지는 개인 책상)
*   **`Heap`**: 힙. (동적으로 생성된 무거운 객체나 배열이 마음대로 널브러져 저장되는 거대한 물류 창고)
*   **`Garbage Collector (GC)`**: 가비지 컬렉터. (힙 영역에서 더 이상 아무도 참조하지 않는 쓰레기 객체를 자동으로 수거해 청소해주는 고마운 자바 청소부)
