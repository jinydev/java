...
*   **`Array`**: 배열. (동일한 타입의 데이터를 연속된 메모리 공간에 나란히 저장하는 구조)
*   **`Index`**: 인덱스, 색인, 순번. (배열의 각 항목 방에 접근하기 위한 번호. 주의: 1이 아니라 0부터 시작!)
*   **`Length`**: 길이. (배열 전체 기차의 칸 수나 항목의 개수)
*   **`Element`**: 요소, 항목, 원소. (배열 칸 안에 저장된 각각의 실제 데이터 값)
*   **`Exception`**: 예외. (프로그램 실행 중 발생하는 예기치 못한 에러 상황. 예: ArrayIndexOutOfBoundsException)
