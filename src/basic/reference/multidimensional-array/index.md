...
*   **`Multidimensional`**: 다차원의. (1차원 길이를 넘어 표(2차원)나 로고스 블록(3차원) 구조 형태를 갖는 배열)
*   **`Row / Column`**: 로우 / 컬럼. (2차원 배열에서 각각 가로줄(행/층수)과 세로줄(열/호수)을 의미)
*   **`Jagged Array`**: 재기드 어레이. (계단식 배열. 건물 층마다 방의 개수가 삐뚤빼뚤하게 다를 수 있는 구조의 배열)
