...
description: "1.4 바이트코드와 JVM 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "1.4 바이트코드와 JVM, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Virtual Machine`**: 버추얼 머신, 가상 머신. (물리적 컴퓨터가 아니라, 소프트웨어로 구현된 가짜 컴퓨터 환경)
*   **`Bytecode`**: 바이트코드. (자바 소스를 컴파일하여 만들어진, JVM만이 통역할 수 있는 중간 단계의 공용어 파일)
*   **`Compile`**: 컴파일, 번역하다. (사람이 작성한 소스 코드를 컴퓨터나 가상 머신이 밥으로 먹을 수 있는 코드로 변환하는 작업)