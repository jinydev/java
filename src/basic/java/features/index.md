...
*   **`Feature`**: 피처, 특징, 기능. (자바 언어만의 고유한 장점이나 특별한 성질)
*   **`Portability`**: 포터블리티, 이식성. (윈도우, 맥 등 플랫폼에 구애받지 않고 어디서든 이사 가서 실행할 수 있는 성질)
*   **`Object-Oriented (OO)`**: 오브젝트 오리엔티드, 객체지향. (프로그램을 독립된 부품(객체)들의 조립품으로 보는 현대적인 설계 방식)
*   **`Garbage Collector (GC)`**: 가비지 컬렉터, 쓰레기 수집기. (더 이상 사용하지 않는 메모리를 알아서 찾아 싹 치워주는 자바 안의 지능형 청소 로봇)