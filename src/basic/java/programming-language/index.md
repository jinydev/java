...
description: "1.1 프로그래밍 언어와 자바 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "1.1 프로그래밍 언어와 자바, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Programming Language`**: 프로그래밍 랭귀지, 프로그래밍 언어. (사람과 컴퓨터가 대화할 수 있게 도와주는 통역사 역할의 약속된 언어)
*   **`Machine Language`**: 머신 랭귀지, 기계어. (컴퓨터가 직접 물리적으로 이해할 수 있는 0과 1로만 이루어진 가장 원초적인 언어)
*   **`High/Low-Level`**: 하이/로우 레벨, 고급/저급. (사람이 이해하기 쉬울수록 고차원(High), 기계에 가까울수록 저차원(Low)의 언어라고 부름)
