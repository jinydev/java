...
*   **`Character (char)`**: 캐릭터, 문자. (작은따옴표(`' '`)로 단단히 묶어 딱 한 글자만 저장할 수 있는 전용 문자 데이터 타입)
*   **`Unicode`**: 유니코드. (컴퓨터가 글자를 이해할 수 있도록 전 세계의 모든 문자에 숫자 번호를 고유하게 매겨놓은 국제 표준 약속)
*   **`Empty`**: 엠프티, 비어있는. (`char` 타입에는 `''`처럼 아무것도 없는 텅 빈 공간 그 자체를 넣을 수는 없음을 명심하세요)
