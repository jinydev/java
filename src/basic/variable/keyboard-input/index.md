...
*   **`Scanner`**: 스캐너, 훑어보는 기계. (키보드로 빙글빙글 들어오는 날것의 입력 신호를 슥 훑고 읽어서 컴퓨터가 즉시 이해하는 예쁜 데이터로 번역해 주는 전문 번역가 로봇)
*   **`Input / Output (I/O)`**: 인풋 / 아웃풋, 입력 / 출력. (컴퓨터 밖(키보드)에서 안으로 신경을 밀어넣는 것을 입력, 안에서 밖(모니터)으로 화려하게 보여주는 것을 출력이라 함)
*   **`Buffer`**: 버퍼, 완충 지대 / 임시 보관함. (키보드 입력을 바로바로 조급히 처리하지 않고 엔터(`\n`)가 통쾌하게 들어올 때까지 잠시 여유롭게 먼저 쌓아두는 넉넉한 바구니 같은 방파제)
