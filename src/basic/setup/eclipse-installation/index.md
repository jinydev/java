...
description: "3. 이클립스 설치 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "3. 이클립스 설치, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Eclipse`**: 이클립스, 일식. (자바 개발자들이 가장 많이 쓰는 유명한 무료 코딩 프로그램 이름)
*   **`IDE (Integrated Development Environment)`**: 아이디이, 통합 개발 환경. (메모장, 번역기, 실행기 방식을 하나로 묶어놓은 코딩 전용 종합 프로그램)
*   **`Workspace`**: 워크스페이스, 작업 공간. (내가 짠 수많은 코드 파일들과 프로젝트가 한데 모여 저장되는 기본 폴더)
