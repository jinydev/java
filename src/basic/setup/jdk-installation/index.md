...
*   **`JDK (Java Development Kit)`**: 제이디케이, 자바 개발 키트. (자바로 프로그램을 만들기 위해 필요한 컴파일러, 디버거 등 모든 도구가 들어있는 실전 종합 세트)
*   **`JRE (Java Runtime Environment)`**: 제이알이, 자바 실행 환경. (자바 프로그램을 만들지 않고 오직 실행하기만 할 때 필요한 최소한의 부품 모음)
*   **`LTS (Long Term Support)`**: 엘티에스, 장기 지원. (오랜 기간 동안 안정적으로 치명적 오류를 고쳐주며 책임을 지겠다고 공식으로 약속한 버전)
