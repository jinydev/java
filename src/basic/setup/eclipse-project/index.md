...
*   **`Project`**: 프로젝트. (하나의 거대한 프로그램을 완성하기 위해 관련된 모든 파일, 이미지, 폴더를 모아두는 가장 큰 분류 가방)
*   **`Environment`**: 인바이런먼트, 환경. (프로그램이 실행되기 위해 필요한 다양한 기본 세팅, 조건이나 무대)
*   **`Preference`**: 프리퍼런스, 환경설정. (프로그램의 기본 동작 방식이나 글꼴, 화면 테마 등을 내 입맛대로 바꾸는 설정 메뉴)
