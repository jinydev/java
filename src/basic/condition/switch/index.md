...
*   **`Switch`**: 스위치, 전환하다. (변수의 상태 값에 따라 실행할 번호(case)로 곧바로 전환)
*   **`Case`**: 케이스, 경우. (switch문 안에서 찾아갈 각각의 목적지 번호표)
*   **`Default`**: 디폴트, 기본값. (모든 case에 해당하지 않을 때 기본적으로 실행되는 곳)
*   **`Break`**: 브레이크, 탈출하다. (현재 몸담고 있는 switch나 반복문의 블록을 산산조각 내고 빠져나감)
*   **`Fall-through`**: 폴스루, 폭포수 현상. (break를 쓰지 않아 아래의 case들까지 멈추지 않고 연달아 실행되는 현상)
