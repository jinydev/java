...
*   **`Overflow`**: 오버플로우, 넘쳐흐르다. (변수의 최댓값을 초과하여 거꾸로 최솟값으로 되돌아가는 치명적 버그 현상)
*   **`Underflow`**: 언더플로우, 아래로 넘치다. (변수의 최솟값보다 작아지려 할 때 거꾸로 최댓값으로 되돌아가는 현상)
*   **`Most Significant Bit (MSB)`**: 엠에스비, 최상위 비트. (비트 구조 중 가장 맨 앞에 위치하여 숫자의 부호(+/-)를 결정하는 중요한 비트)
