...
*   **`Sign`**: 사인, 부호. (`+` 또는 `-` 처럼 숫자의 양/음 상태를 나타내는 기호)
*   **`Increment`**: 인크리먼트, 증가. (값을 1씩 올리는 단항 연산 `++`)
*   **`Decrement`**: 디크리먼트, 감소. (값을 1씩 내리는 단항 연산 `--`)
*   **`Prefix / Postfix`**: 프리픽스 / 포스트픽스. (전위 / 후위. 연산자가 변수 이름 앞에 붙느냐 뒤에 붙느냐에 따른 실행 순서 위치 차이)
