...
*   **`Comparison`**: 컴패리즌, 비교. (두 값의 크기나 생김새가 똑같은지 저울질해보는 것)
*   **`Equals`**: 이퀄즈, 같다. (`==`와 달리 포장된 상자 안의 진짜 내용물이 완전히 똑같은 상태인지 묻는 명령어)
*   **`Greater/Less`**: 그레이터/레스, 크다/작다.
*   **`Casting`**: 캐스팅, 강제 형변환. (데이터의 타입을 다른 타입으로 억지로 둔갑시키는 행위)