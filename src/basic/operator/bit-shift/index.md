...
*   **`Shift`**: 시프트, 이동하다. (메모리에 있는 비트 0, 1 조각들을 왼쪽이나 오른쪽 칸으로 쓱 밀어버리는 동작)
*   **`Binary`**: 바이너리, 이진수. (컴퓨터가 기계에서 사용하는 0과 1로만 이루어진 2진수 숫자 체계)
*   **`Sign Bit`**: 사인 비트, 부호 비트. (가장 맨 앞자리에 위치하여 숫자가 양수인지 음수인지 결정하는 특별한 비트)
