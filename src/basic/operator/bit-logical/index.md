...
| XOR (배타적 논리합) | `^`    | 두 비트 중 다를 경우(하나만 1) 연산 결과는 1               |
| NOT (논리 부정)     | `~`    | 0은 1로, 1은 0으로 반전 (보수)                             |

### 논리 게이트 시각화 💡

비트 논리 연산자는 실제 컴퓨터 CPU 내부의 반도체(트랜지스터) 회로에서 전기가 통하는 원리를 코드로 옮겨 놓은 것입니다! 1은 전기가 통함(ON), 0은 안 통함(OFF)을 의미합니다.

![비트 논리 게이트 애니메이션](./img/bit_logic_gates.svg)
...
*   **`Bit`**: 비트. (컴퓨터 데이터의 가장 작은 단위, 0과 1 표시)
*   **`Logical`**: 로지컬, 논리적인. (컴퓨터 회로에서 전기가 통하냐 안 통하냐의 참/거짓 판단)
*   **`Unsigned`**: 언사인드, 부호가 없는. (마이너스(-) 기호 없이 양수만 표현하는 크기가 더 큰 자료형)
*   **`Exclusive (XOR)`**: 익스클루시브, 배타적인. (두 값이 서로 다를 때만 참을 반환하는 특수 논리 연산)
