...
*   **`Precedence`**: 프리시던스, 우선순위. (여러 연산자가 한 줄에 섞여 있을 때 먼저 계산해야 할 알맞은 VIP 서열)
*   **`Associativity`**: 어소시에이티비티, 결합 법칙 (방향). (우선순위가 동급인 연산자들끼리 만났을 때 왼쪽부터 할지 오른쪽부터 할지 결정하는 방향)
*   **`Parentheses`**: 퍼렌서시스, 소괄호 `()`. (모든 우선순위 서열을 무시하고 가장 최우선으로 먼저 묶어서 계산하도록 강제하는 기호)
