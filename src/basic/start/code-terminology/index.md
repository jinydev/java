...
description: "3.2 코드 용어 이해 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "3.2 코드 용어 이해, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Package`**: 패키지, 꾸러미. (비슷한 종류의 클래스 파일들을 명칭 충돌 없이 하나로 묶어 관리하는 폴더)
*   **`Class`**: 클래스, 반/학급. (자바 프로그램의 기본 단위이자, 객체를 만들어내기 위한 필수적인 붕어빵 틀(설계도))
*   **`Main Method`**: 메인 메소드. (자바 가상 머신(JVM)이 프로그램을 실행할 때 가장 먼저 찾아서 진입하는 시작점, 자동차의 시동 버튼)
