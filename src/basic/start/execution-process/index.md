...
description: "3.1 소스 작성부터 실행까지 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "3.1 소스 작성부터 실행까지, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Source`**: 소스, 원본. (사람이 이해할 수 있는 자바 언어로 직접 타이핑하여 작성한 `.java` 텍스트 원본 파일)
*   **`Compile`**: 컴파일, 번역하다. (사람이 쓴 코드를 컴퓨터가 알아들을 수 있는 기계어 형태에 가깝게 번역하는 과정)
*   **`Bytecode`**: 바이트코드. (자바 소스를 컴파일하여 만들어진, JVM만이 읽고 마저 번역할 수 있는 `.class` 형태의 중간 파일)
