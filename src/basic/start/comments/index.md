...
![그림](./img/single_line_comment.png)   
...
*   **`Comment`**: 코멘트, 주석. (컴파일러는 무시하고 사람(개발자)만이 읽을 수 있도록 코드에 남겨두는 친절한 설명 메모)
*   **`Slash`**: 슬래시 표시 `/`. (한 줄짜리 짧은 주석을 시작할 때 두 번 연달아 `//` 사용하는 기호)
*   **`Asterisk`**: 애스터리스크, 별표 `*`. (여러 줄의 긴 주석을 묶을 때 슬래시와 함께 `/* ... */` 형태로 사용하는 기호)
