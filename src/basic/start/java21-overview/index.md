...
description: "1.5 자바 21 개요 에 대한 자바(Java) 기초 및 실전 프로그래밍 문서입니다."
keywords: "1.5 자바 21 개요, 자바, Java, 프로그래밍, 백엔드, 개발, jinydev"
---
...
*   **`Virtual Thread`**: 버추얼 스레드, 가상 스레드. (적은 메모리로도 수만 개의 작업을 동시에 처리할 수 있게 해주는 혁신적인 경량 작업 흐름 단위)
*   **`Sequence / Collection`**: 시퀀스 / 컬렉션, 순서 / 수집품. (데이터들이 순서대로 잘 정리된 형태, 첫 번째(`getFirst`)나 마지막(`getLast`) 데이터를 쉽게 꺼낼 수 있음)
*   **`Record / Pattern`**: 레코드 / 패턴, 기록 / 형태. (객체 내부의 데이터를 투시하듯 한 번에 변수로 바로 뽑아내는 최신 매칭 기법)
